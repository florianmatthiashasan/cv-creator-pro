  ...
  return (
    <div className="space-y-6">
      <AnimatePresence mode="popLayout">
        {data.map((edu, index) => (
          <motion.div key={edu.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="soft-panel relative p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="section-kicker">Bildung</p>
                <span className="mt-1 inline-block text-sm font-display text-primary">Eintrag {index + 1}</span>
              </div>
